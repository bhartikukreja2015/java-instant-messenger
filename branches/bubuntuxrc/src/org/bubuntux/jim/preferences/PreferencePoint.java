package org.bubuntux.jim.preferences;
...
public class PreferencePoint {
	...
	public void saveAccount(AccountSettings myAccount) {
		Preferences accountNode = myPrefs.node("Accounts").node(
				"" + myAccount.getID());

		accountNode.put(PreferencePoint.usernameKey, myAccount.getUsername());
		if (myE != null) {
			accountNode.putByteArray(PreferencePoint.passwordKey, myE
					.encrypt(myAccount.getPassword().getBytes()));
		} else {
			accountNode.put(PreferencePoint.passwordKey, myAccount
					.getPassword());
		}
		accountNode.put(PreferencePoint.accountTypeKey, myAccount
				.getAccountType());
		accountNode.putBoolean(PreferencePoint.enabledKey, myAccount
				.isEnabled());
	...
	public ArrayList<AccountSettings> getAllAccounts() {
		...
		try {
			for (String s : accountNode.childrenNames()) {
				...
				myA.setUsername(accountNode.node(s).get(
						PreferencePoint.usernameKey, ""));
				if (myE != null) {
					myA.setPassword(new String(myE.decrypt(accountNode.node(s)
							.getByteArray(PreferencePoint.passwordKey, null))));
				} else {
					myA.setPassword(accountNode.node(s).get(
							PreferencePoint.passwordKey, ""));
				}
				myA.setAccountType(accountNode.node(s).get(
						PreferencePoint.accountTypeKey, ""));
				myA.setEnabled(accountNode.node(s).getBoolean(
						PreferencePoint.enabledKey, false));
				myA.setAlias(accountNode.node(s).get(
						PreferencePoint.accountAlias, ""));
	...
	public AccountSettings getAccount(int id) {
		...
		try {
			...
			if (myE != null) {
				myA.setPassword(new String(myE.decrypt(accountNode
						.getByteArray(PreferencePoint.passwordKey, null))));
			} else {
				myA.setPassword(accountNode
						.get(PreferencePoint.passwordKey, ""));
			}
			myA.setAccountType(accountNode.get(PreferencePoint.accountTypeKey,
					""));
			myA.setEnabled(accountNode.getBoolean(PreferencePoint.enabledKey,
					false));
	...
	public void setAliasForScreenname(String screenname, String accountName,
			String alias) {
		Preferences aliasNode = myPrefs.node("Aliases");
	...
	public void setMergeIDForScreenname(String screenname, String accountName,
			int mergeID) {

		// System.out.println("Setting merge ID of " + screenname + " to " +
		// mergeID);
