...
public class YahooAccount implements AbstractAccount, SessionListener,
		...
		try {
			...
			if (theStatus.getStatusMessage() == null) {
				if (theStatus.getStatus().equals(Status.available)
					...
					System.out.println("Got unknown status in Yahoo: "
							+ theStatus.getStatus());
				}
			} else {
				// it is a custom status.
				boolean b = (theStatus.getStatus().equals(Status.available) || theStatus
						.getStatus().equals(Status.superAvailable));
	...
	// surpressing the setting of the theUsers variable... we know we are
	// getting data of type YahooUser
	...
	protected Buddy YahooUserToJimBuddy(YahooUser myYU) {
		...
		} else if (myYU.getStatus() == StatusConstants.STATUS_BRB
				|| myYU.getStatus() == StatusConstants.STATUS_NOTATDESK
				|| myYU.getStatus() == StatusConstants.STATUS_NOTATHOME
				|| myYU.getStatus() == StatusConstants.STATUS_NOTINOFFICE
				|| myYU.getStatus() == StatusConstants.STATUS_ONPHONE
				|| myYU.getStatus() == StatusConstants.STATUS_OUTTOLUNCH
				|| myYU.getStatus() == StatusConstants.STATUS_STEPPEDOUT) {
	...
	public void gotPacket(YMSG9Packet arg0) {
		...
		if (arg0.service == 1 && arg0.status == 0 && arg0.body[0].equals("0")) {
			...
			if (arg0.body[15].equals("1"))
				return;

			System.out.println(arg0.toString());

			// make sure we don't wipe out this value when the broken event
			// comes in
			ignoreNext.addUser(arg0.body[7]);

			// and, we'll also get a friendsUpdate event, so we need to ignore
			// both
			...
			Status toSet = new Status();
			toSet.setStatus((arg0.body[16].equals("138") ? Status.available
					: Status.away));
		...
		if (arg0.status == 1 && arg0.body[0].equals("7")) {
			...
			toSet.setStatus((arg0.body[9].equals("0") ? Status.available
					: Status.away));
