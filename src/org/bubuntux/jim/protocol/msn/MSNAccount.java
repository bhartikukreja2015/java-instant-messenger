...
public class MSNAccount implements AbstractAccount, MsnMessageListener,
		MsnMessengerListener, MsnContactListListener {
	...
	public void addBuddy(Buddy theBuddy) {
		myCon.addFriend(Email.parseStr(theBuddy.getScreename()), theBuddy
				.getAlias());
	}

	public void connect() {
		myCon = MsnMessengerFactory.createMsnMessenger(theAS.getUsername(),
				theAS.getPassword());
	...
	protected Buddy MSNContactToJimBuddy(MsnContact myMC) {
		...
		// System.out.println(myStatus.getDisplayStatus() + "|" +
	...
	protected void dispatchContactList(MsnContactList theList, boolean firstTime) {
		...
		for (MsnContact myMC : theContacts) {
			...
			if (theStopper.checkBuddy(myMC.getId())) {
				theEvents.buddyStatusChange(this.MSNContactToJimBuddy(myMC),
						firstTime);
			}
		}
	}
	...
	} // at this point in Jim, we'll just cry.
